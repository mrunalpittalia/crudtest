...
<div class="row g-0 justify-content-center">
    <div class="col-12 col-md-10">
        <div class="card border-0 bg-transparent">
            <form action="{{ route('company.update', $company->company_id) }}" class="needs-validation" method="POST" novalidate name="company_edit_form" id="company_edit_form" >
                ...
                <div class="card-body border-0 px-0">
                    <div class="row">
                        <div class="mb-3 col-12 col-md-6">
                            ...
                            <input type="text" class="form-control{{ $errors->has('company_name') ? ' is-invalid' : '' }}" id="company_name" name="company_name" value="{{ $company->company_name }}" />
                        ...
                        <div class="mb-3 col-12 col-md-6">
                            ...
                            <select class="form-select{{ $errors->has('company_type') ? ' is-invalid' : '' }}" id="company_type" name="company_type" >
                                ...
                                <option value="1" @if($company->company_type == 1){{ ' selected="selected"' }}@endif >{{trans('messages.companies_form_field_type_public_limited_option_text')}}</option>
                                <option value="2" @if($company->company_type == 2){{ ' selected="selected"' }}@endif >{{trans('messages.companies_form_field_type_private_limited_option_text')}}</option>
                                <option value="2" @if($company->company_type == 3){{ ' selected="selected"' }}@endif >{{trans('messages.companies_form_field_type_registered_company_option_text')}}</option>
                            </select>
                        ...
                        <div class="mb-3 col-12  col-md-6">
                            ...
                            <input type="text" class="form-control{{ $errors->has('website') ? ' is-invalid' : '' }}" id="website" name="website" value="{{ $company->website }}" />
                        ...
                        <div class="mb-3 col-12">
                            ...
                            <textarea class="form-control{{ $errors->has('company_description') ? ' is-invalid' : '' }}"
                                name="company_description" id="company_description" rows="3">@if($company->company_description){{ $company->company_description }}@endif</textarea>
                ...
                <div class="card-footer bg-transparent p-0 d-grid gap-1">
                    <button class="btn btn-primary fw-bold border-0 text-white" type="submit" style="outline: none;">
                        {{ trans('messages.company_edit_form_submit_btn_text') }}
                    </button>
                </div>
                @csrf
                @method('PUT')
